...
for i in range(len(calls)):
    ...
    candidate.append(calls[i][0])
candidate = sorted(list(set(candidate)))
#删除以上得到的推销员列表中发送短信，接收短信或者接收电话的号码
for phonecall in candidate:
    if (phonecall not in recall) and (phonecall not in sendtexts) and (phonecall not in retexts):
        tui.append(phonecall)
#逐行输出结果
